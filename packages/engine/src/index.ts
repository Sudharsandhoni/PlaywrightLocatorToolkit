...
Incorrect: .or.locator('...')  ←  missing parentheses and argument\nCorrect:   .or(locator('...'))  ←  pass the alternative locator as argument`;
  ...
  async simulateHover(pageId: string, locatorStr: string): Promise<boolean> {
    const page = this.getPage(pageId);
    if (!page) return false;
    try {
      await this.ensureAgentInjected(page);
      ...
      return await page.evaluate((el: any) => {
        return (window as any).__locatorLensAgent.simulateHover(el);
      }, handle);
    } catch (err) {
...
`;
  ...
  async executeExtensionSandbox(
    ...
    try {
      let locatorInstance: any = undefined;
  ...
  prepareWorkspaceScript(
    ...
    if (customTempDir) {
      ...
      let resolvedSpecDir: string | null = null;
...
export function generateWorkspaceScriptContent(
  userCode: string,
  mode: 'playwright-test' | 'standalone',
  attachCdp: boolean,
  cdpUrl?: string,
  targetUrl?: string,
  isTypeScript = true
...
export function prepareWorkspaceScript(
  userCode: string,
  mode: 'playwright-test' | 'standalone',
  attachCdp: boolean,
  cdpUrl?: string,
